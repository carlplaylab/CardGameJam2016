...
public class TextMeshOutline : MonoBehaviour 
{
	...
	void Start() 
	{
		...
		SortLayerModifier adjuster = GetComponent<SortLayerModifier>();
		...
		for (int i = 0; i < 8; i++) {
			...
			otherMeshRenderer.useLightProbes = false;
			otherMeshRenderer.sortingLayerID = meshRenderer.sortingLayerID;
			otherMeshRenderer.gameObject.layer = meshRenderer.gameObject.layer;

			if(adjuster != null)
			{
				otherMeshRenderer.sortingLayerName = adjuster.SortingLayerName;
				otherMeshRenderer.sortingOrder = adjuster.SortingLayer-1;
			}
			else
			{
